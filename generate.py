...
def generate_ninja(filename: str, modulesToBePreCompiledByEachSource: dict[str, modulesDependency]):
    ...
    with stdout if filename == '--'else open(filename, "w") as fout:
        ...
        out.variable("cxx", r"D:/msys2/mingw64/bin/g++.exe")
        if compiler == "clang":
            ...
            out.variable('interface', '--precompile -x c++-module')
            out.variable('ext', 'pcm')
        elif compiler == "cl":
            out.variable("standard", "/std:c++20")
        ...
        for source, modulesToBePreCompiled in modulesToBePreCompiledByEachSource.items():
            out.build("$builddir/"+source, "cxx",
                      [modulesBiDict[moduleToBePreCompiled] for moduleToBePreCompiled in modulesToBePreCompiled.module] +
                      [source])
